...
(function () {
  ...
  function ManyMany($uibModal, $compile, $timeout) {
    ...
    return {
      ...
      transclude: true,
      link: function link(scope, elm, attrs, ctrl, transcludeFn) {
        ...
        scope.halp = function (obj) {
          scope.template = '<div class="modal-body">\n';
          for (var key in obj) {
          ...
          var mi = $uibModal.open({
            ...
            size: 'sm',
            controller: function controller($scope, $value, $uibModalInstance) {
              $scope.$value = $value;
              $scope.back = function () {
  ...
  angular.module('gumga.manytomany', ['ui.bootstrap']).directive('gumgaManyToMany', ManyMany);
})();
